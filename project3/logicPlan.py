...
def checkLocationSatisfiability(x1_y1: Tuple[int, int], x0_y0: Tuple[int, int], action0, action1, problem):
    ...
    model_true = findModel(conjoin(KB + [PropSymbolExpr(pacman_str, x1, y1, time=1)]))
    model_false = findModel(conjoin(KB + [~PropSymbolExpr(pacman_str, x1, y1, time=1)]))
...
def positionLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        KB.append(exactlyOne([PropSymbolExpr(act, time=t) for act in DIRECTIONS]))

        for x, y in non_wall_coords:
            KB.append(pacmanSuccessorAxiomSingle(x, y, time=t+1, walls_grid=walls_grid))
 
        cal = findModel(conjoin(KB) & PropSymbolExpr(pacman_str, xg, yg, time=t))
        if cal:
            return extractActionSequence(cal, actions)
    "*** END YOUR CODE HERE ***"
...
def foodLogicPlan(problem) -> List:
    ...
    for t in range(50):
        ...
        temp = []
        for (x, y) in food:
            temp.append(PropSymbolExpr(food_str, x, y, time=t))    
        cal = findModel(conjoin(KB) & ~disjoin(temp))
        if cal:
            return extractActionSequence(cal, actions)
    # util.raiseNotDefined()
    "*** END YOUR CODE HERE ***"
...
def localization(problem, agent) -> Generator:
    ...
    "*** BEGIN YOUR CODE HERE ***"
    (KB.append(PropSymbolExpr(wall_str, x, y)) for x, y in walls_list) if (x,y) in all_coords else KB.append(~PropSymbolExpr(wall_str, x, y))

    possible_locations = non_outer_wall_coords
    # util.raiseNotDefined()

    for t in range(agent.num_timesteps):
        action = agent.getAction(t)
        percept = agent.getPercept(t)
        percept_expr = PropSymbolExpr('sensor' + str(t), percept)
        KB.append(pacphysicsAxioms(action, t) & percept_expr)

        new_possible_locations = []
        for x, y in possible_locations:
            location_expr = PropSymbolExpr(pacman_str, x, y, t)
            if pl_true(conjoin(KB), location_expr):
                new_possible_locations.append((x, y))
