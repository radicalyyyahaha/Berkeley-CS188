...
def constructBayesNet(gameState: hunters.GameState):
    ...
    Set = set()
    for a in [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]:
        for b in [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]:
            manh = manhattanDistance(a, b)
            for obsv in range(max(0, manh - MAX_NOISE), manh + MAX_NOISE + 1):
                Set.add(obsv)
    variableDomainsDict[OBS0] = list(Set)
    variableDomainsDict[OBS1] = list(Set)
    # raiseNotDefined()
    "*** END YOUR CODE HERE ***"
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for eliminationVariable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) > 1:
                el = eliminate(joinedFactor, eliminationVariable)
                currentFactorsList.append(el)
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.allPositions:
            self.beliefs[pos] *= self.getObservationProb(observation, p, pos, jail)
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState: busters.GameState):
        ...
        self.particles = []
        "*** YOUR CODE HERE ***"
        a = len(self.legalPositions)
        b = self.numParticles
        # c = b // a
        for pos in range(b):
            self.particles.append(self.legalPositions[pos % a])
    ...
    def getBeliefDistribution(self):
        ...
        for pos in self.legalPositions:
            dist[pos] = self.particles.count(pos)
        dist.normalize()
        return dist
        # raiseNotDefined()
        "*** END YOUR CODE HERE ***"
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for pos in self.particles:
            new[pos] += self.getObservationProb(observation, p, pos, jail)
        new.normalize()
        if new.total() == 0:
            self.initializeUniformly(gameState)
        else:
